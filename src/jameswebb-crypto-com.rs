...
fn log_event(payload: &str) {
    ...
    if let Some(method) = value.get("method") {
        eprintln!("crypto.com method: {}", method);
    }
}

fn to_crypto_com_channel(symbol: &str) -> String {
    let (base, quote) = split_symbol(symbol);
    if quote.is_empty() {
        format!("ticker.{}", symbol.to_uppercase())
    } else {
        format!("ticker.{}_{}", base.to_uppercase(), quote.to_uppercase())
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let symbol = env::args()
        ...
        .to_lowercase();
    let channel = to_crypto_com_channel(&symbol);
